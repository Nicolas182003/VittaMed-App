...
import com.example.vittamed.io.response.SimpleResponse
import com.example.vittamed.model.Doctor
import com.example.vittamed.model.Schedule
import com.example.vittamed.model.Specialty
        ...
        onBackPressedDispatcher.addCallback(this, object : androidx.activity.OnBackPressedCallback(true){
            override fun handleOnBackPressed() {
                if (cvResumen.visibility == View.VISIBLE){
                    ...
                    val builder = AlertDialog.Builder(this@CreateAppointmentActivity)
                    builder.setTitle("Estas seguro que desea salir?")
                    builder.setMessage("Si abandonas el resgitro, los datos que habia ingresado se perderan.")
    ...
    private fun loadSpecialties(){
        ...
        call.enqueue(object: Callback<ArrayList<Specialty>>{
            override fun onResponse(
                ...
                if (response.isSuccessful){
                    response.body()?.let {
                        val  specialty = it.toMutableList()
                        spinnerSpecialties.adapter = ArrayAdapter(this@CreateAppointmentActivity, android.R.layout.simple_list_item_1, specialty)
                    }
                }
            }
